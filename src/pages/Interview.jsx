...
function Interview({ user, restart }) {
    ...
    if (index === ques.length - 1) {
      localStorage.setItem(
        ...
        JSON.stringify({
          name: user.name,
          topic: user.topic,
          score: score + (isCorrect ? 1 : 0), // ✅ FIXED
  ...
  const history = JSON.parse(localStorage.getItem("interviewResult") || "null");
      ...
      <div className="container">
        ...
        {allAnswers.map((ans, i) => (
          <div key={i} className="result-card">
            <p><strong>Q{i + 1}:</strong> {ques[i]?.question}</p>
            <p><strong>Your Ans:</strong> {ans}</p>
            <p><strong>Correct Ans:</strong> {ques[i]?.answer}</p>
            <p>
...
<div className="progress-container">
  ...
  ></div>
      ...
      <h4>Question {index + 1} / {ques.length}</h4>
